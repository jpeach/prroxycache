...
impl Vol {
    pub fn from_block(block: &disk::SpanBlock) -> Self {
    ...
    /// bucket_count is the total number of buckets, distributed equally
    ...
    fn header_offsets(self: &Self, base: u64) -> (u64, u64, u64, u64) {
        let header_length: u64 = disk::VolHeaderFooter::SIZE_BYTES as u64;
...
fn directory_length(segment_count: u64, buckets_per_segment: u64) -> u64 {
    ...
    let directory_length: u64 = StoreBlocks::from(Bytes::from(
        buckets_per_segment * disk::ENTRIES_PER_BUCKET * segment_count * disk::ENTRY_SIZE,
    ))
    .as_bytes();
    let footer_length: u64 =
        StoreBlocks::from(Bytes::from(disk::VolHeaderFooter::SIZE_BYTES as u64)).as_bytes();
...
fn converge_header_info(
    ...
    let total_buckets: u64 = total_entries / disk::ENTRIES_PER_BUCKET;

    // Work out how many segment needed to cover all the buckets.
    segment_count =
        (total_buckets + disk::MAX_BUCKETS_PER_SEGMENT - 1) / (disk::MAX_BUCKETS_PER_SEGMENT);
